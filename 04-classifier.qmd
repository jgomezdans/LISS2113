...
In my data set, the overall accuracy was around 82%, with a $\kappa$ value around 0.74, which suggests that on the same year, and around Bamako, the classifier is pretty good. The confusion matrix gives as an interesting vista of the classifier's performance:

We can make the following observations:

* `Water`: This is the most accurately classified category, with 47 out of 48 instances correctly identified.
* `Built up: This class shows strong performance with 114 correct predictions, though it occasionally absorbs misclassifications from Bare Soil and Veg.
