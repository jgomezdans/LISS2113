...
## Summary
```{mermaid}
%%| fig-cap: "Urban Growth Analysis: Systematic Workflow"
%%| fig-width: 10

flowchart LR
    %% Data Phase
    subgraph Data ["DATA INPUTS"]
        A1[("Landsat 5-9<br/>L2 SR")] 
        A2[("Sentinel-2<br/>MSI")]
    end

    %% Processing Phase
    subgraph Prep ["PRE-PROCESSING"]
        B1{Filter} --> B2(Harmonize)
        B2 --> B3(Median Composite)
    end

    %% Analysis Phase
    subgraph ML ["MACHINE LEARNING"]
        C1[/Training Labels/] --> C2[[Random Forest]]
        B3 --> C2
        C2 --> C3{Validation}
    end
    ...
    %% Styling - This is the "Boring-Prevention" section
    style Data fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    style Prep fill:#fffde7,stroke:#fbc02d,stroke-width:2px
    style ML fill:#e3f2fd,stroke:#1976d2,stroke-width:2px
    style Out fill:#ffebee,stroke:#c62828,stroke-width:2px
    
    %% Specific Highlight for the Classifier
    style C2 fill:#1976d2,color:#fff,stroke-width:3px
